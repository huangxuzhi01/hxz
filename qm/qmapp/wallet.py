...
import json
import MySQLdb
from qmapp.config import database,sqls
...
def checkUserExists(mobile,qm_env):
    #输入注册手机号，点击下一步#
    URL = database[qm_env][0]['url']
    url = "http://{0}.ddwallet.net/ddwallet-app/user/checkUserExists".format(URL)
    data = {"mobile":mobile}
...
def getValidateCode(mobile,qm_env):
    #获取验证码#
    URL = database[qm_env][0]['url']
    url = "http://{0}.ddwallet.net/ddwallet-app/user/getValidateCode".format(URL)
...
def checkVerifyCode(mobile,qm_env):
    #输入完验证码#
    URL = database[qm_env][0]['url']
    url = "http://{0}.ddwallet.net/ddwallet-app/user/checkVerifyCode".format(URL)
...
def register_contract(qm_env):
    #进入设置密码页#
    URL = database[qm_env][0]['url']
    url = "http://{0}.ddwallet.net/ddwallet-app/contract/register".format(URL)
...
def register(mobile,qm_env):
    #设置密码，点击完成注册#
    URL = database[qm_env][0]['url']
    url = "http://{0}.ddwallet.net/ddwallet-app/user/register".format(URL)
...
def login(mobile,qm_env):
    #输入登录密码，点击下一步#
    URL = database[qm_env][0]['url']
    url = "http://{0}.ddwallet.net/ddwallet-app/user/login".format(URL)
...
def analisis(sessionid,qm_env):
    ...
    URL = database[qm_env][0]['url']
    url = "http://{0}.ddwallet.net/ddwallet-app/api/qmwallet/analisis".format(URL)
    data = {
...
def locatingInfo(sessionid,qm_env):
    ...
    URL = database[qm_env][0]['url']
    url = "http://{0}.ddwallet.net/ddwallet-app/user/locatingInfo".format(URL)
    data = {
        "detailedAddress":"",
        "province":"",
...
def deviceInfo(sessionid,qm_env):
    ...
    URL = database[qm_env][0]['url']
    url = "http://{0}.ddwallet.net/ddwallet-app/user/deviceInfo".format(URL)
    data = {
        "sysVersion":"7.1.1",
        "appVersion":"2.3.1.1",
        "phoneType":"OPPO R11",
...
def saveLocalInfo1(sessionid,qm_env):
    ...
    URL = database[qm_env][0]['url']
    url = "http://{0}.ddwallet.net/ddwallet-app/user/saveLocalInfo".format(URL)
    data = {
        "message":"[]",
        "infoType":1
    }
...
def font_realNameAuth(sessionid,qm_env):
    URL = database[qm_env][0]['url']
    url = "http://{0}.ddwallet.net/ddwallet-app/amount/realNameAuth".format(URL)
    headers2 = {
        "appKey": "QmWallet",
        "channel": "360PT",
        "device": "android",
...
def saveRealNameAuth(id_card,sessionid,qm_env):
    URL = database[qm_env][0]['url']
    url = "http://{0}.ddwallet.net/ddwallet-app/amount/saveRealNameAuth".format(URL)
    headers2 = {
        "appKey": "QmWallet",
        "channel": "360PT",
        "device": "android",
...
def faceIdAuth(sessionid,qm_env):
    URL = database[qm_env][0]['url']
    url = "http://{0}.ddwallet.net/ddwallet-app/amount/faceIdAuth".format(URL)
    headers2 = {
        "appKey": "QmWallet",
        "channel": "360PT",
        "device": "android",
...
def handIdCard(sessionid,qm_env):
    URL = database[qm_env][0]['url']
    url = "http://{0}.ddwallet.net/ddwallet-app/amount/handIdCard".format(URL)
    headers2 = {
        "appKey": "QmWallet",
        "channel": "360PT",
        "device": "android",
...
def addressBookAuth(sessionid,qm_env):
    ...
    url = "http://{0}.ddwallet.net/ddwallet-app/amount/addressBookAuth".format(URL)
    data = {
        "authItemCode":"1004",
...
def baseInfoAuth(sessionid,qm_env):
    ...
    url = "http://{0}.ddwallet.net/ddwallet-app/amount/baseInfoAuth".format(URL)
    data = {
...
def amountAssessment(sessionid,qm_env):
    ...
    URL = database[qm_env][0]['url']
    url = "http://{0}.ddwallet.net/ddwallet-app/amount/amountAssessment".format(URL)
    data = {
    }
    data = json.dumps(data)
    resp = requests.post(url=url, data=data, headers=headers2)
...
def genRiskScoreAndCheckUser(sessionid,qm_env):
    ...
    URL = database[qm_env][0]['url']
    url = "http://{0}.ddwallet.net/ddwallet-app/user/genRiskScoreAndCheckUser".format(URL)
    data = {
    }
...
def update_chanel(mobile, chanel_env):
    datas = database[chanel_env][0]
    db = MySQLdb.connect(datas['chanel_db_url'], datas['chanel_db_uname'], datas['chanel_db_pwd'], datas['chanel_db_name'], charset='utf8')
    cursor = db.cursor()
    try:
        cursor.execute(sqls['更新渠道user表用户类型'].format(mobile=mobile))
        cursor.execute(sqls['更新渠道t_loan_apply表'].format(mobile=mobile,Time=cur_time()))
        db.commit()
...
def update_qm(mobile, qm_env,status):
    datas = database[qm_env][0]
    db = MySQLdb.connect(datas['qm_db_url'], datas['qm_db_uname'], datas['qm_db_pwd'], datas['qm_db_name'], charset='utf8')
    cursor = db.cursor()
    try:
        cursor.execute(sqls['查询用户信息'].format(mobile=mobile))
        user_Id = cursor.fetchone()[0]
        cursor.execute(sqls['更新全民库订单表'].format(userid=user_Id, Time=cur_time(), status=status,))
    except Exception as e:
        db.rollback()
    cursor.close()
    db.commit()
...
#获取用户id 和 订单号
def get_user_message(mobile,qm_env):
    msg=[]
    datas = database[qm_env][0]
    db = MySQLdb.connect(datas['qm_db_url'], datas['qm_db_uname'], datas['qm_db_pwd'], datas['qm_db_name'], charset='utf8')
    cursor = db.cursor()
    try:
        ...
        user_Id = cursor.fetchone()[0]
        msg.append(user_Id)
        cursor.execute(sqls['查询全民订单号'].format(userid=user_Id))
...
def bindCard(mobile,sessionid,id_card,bank_card,qm_env):
    ...
    URL = database[qm_env][0]['url']
    url1 = "http://{0}.ddwallet.net/ddwallet-app/order/bindCardValidateCode".format(URL)
    #url1 ="http://t01.hzed.com:133/ddwallet-app/order/bindCardValidateCode"
    data1 = {
        "cardId": "",
        "realName": "朱亚磊",
        "idNumber": id_card,
    ...
    url = "http://{0}.ddwallet.net/ddwallet-app/order/bindCard".format(URL)
    data = {
        "cardId":"",
        "realName":"朱亚磊",
        "idNumber":id_card,
...
#确认借款
def confirmLoan(sessionid,BCbank_card,qm_env,period):
    global order_no
    datas = database[qm_env][0]
    db = MySQLdb.connect(datas['qm_db_url'], datas['qm_db_uname'], datas['qm_db_pwd'], datas['qm_db_name'], charset='utf8')
    cursor = db.cursor()
    ...
    URL = database[qm_env][0]['url']
    url = "http://{0}.ddwallet.net/ddwallet-app/order/confirmLoan".format(URL)
    data = {
        "cardNo":bank_card[-4:],
        "orderNo":order_no,
        "periods":period
...
def status_210(id_card,mobile,chanel_env,qm_env,status):
...
def status_200(id_card,mobile,bank_card,chanel_env,qm_env,status):
    checkUserExists(mobile,qm_env)
    getValidateCode(mobile,qm_env)
    checkVerifyCode(mobile,qm_env)
    register_contract(qm_env)
...
def status_201(id_card,mobile,bank_card,chanel_env,qm_env,status):
    checkUserExists(mobile,qm_env)
    getValidateCode(mobile,qm_env)
    checkVerifyCode(mobile,qm_env)
    register_contract(qm_env)
...
def status_220(id_card,mobile,bank_card,chanel_env,qm_env,status):
    checkUserExists(mobile,qm_env)
    getValidateCode(mobile,qm_env)
    checkVerifyCode(mobile,qm_env)
    register_contract(qm_env)
...
def status_300(id_card,mobile,bank_card,chanel_env,qm_env,status,period):
    checkUserExists(mobile, qm_env)
    getValidateCode(mobile, qm_env)
    checkVerifyCode(mobile, qm_env)
    register_contract(qm_env)
...
if __name__ == '__main__':
    ...
    period = '3'
    status='210'
    status_220(id_card, mobile, bank_card, chanel_env, qm_env, status)
